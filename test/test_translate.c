...
static void test_expand_charseq(void)
{
    ...
    // assert_expand( "z-p", "zabcdefghijklmnop" );
    assert_expand( "\\t", "\t" );
    assert_expand("a-exyz", "abcdexyz");
...
static void test_translate_char(void)
{
    ...
    CHECK_CHAR( translate_char('e', "lee", "LEE"), 'E' );
    CHECK_CHAR( translate_char('z', "123z", "mast"), 't');
...
static void test_translate(void)
{
    ...
    assert_tr( "drink coke everytime", "a-z", "A-Z", "DRINK COKE EVERYTIME");
    assert_tr( "EARN" , "A-Z", "a-z", "earn");
